...
### T-056. 프로덕션 배포
- [x] GitHub 저장소 푸시 완료 (main 브랜치, https://github.com/yeonjin1357/question)
- [ ] Vercel 프로젝트 연결 — **수동 필요**. https://vercel.com/new 에서 `yeonjin1357/question` import. Vercel MCP 는 프로젝트 생성/env 설정 미지원 (deploy_to_vercel 만 있음).
- [ ] 프로덕션 Supabase 분리 — **스킵 결정**. 현재 `qfpkgwuldwjvdgmubhzz` 를 prod 로 그대로 사용 (dev 시드 전량 truncate 완료).
- [ ] Vercel 환경 변수 세팅 — 아래 "필수 env 목록" 참고. `.env.local` 값 그대로 이식하되 `NEXT_PUBLIC_APP_URL`, `DEV_COUNTRY_OVERRIDE`, `SESSION_HASH_SECRET` 는 프로덕션 전용 값으로.
- [ ] **Secrets 로테이트** — 현재는 런칭 직전 수행 예정 (사용자 결정). Supabase 대시보드에서 anon/service_role 재발급 + PAT revoke/재생성.
- [ ] Cloudflare 앞단 구성 (도메인, CF-IPCountry 확인)
- [ ] 첫 24시간 모니터링

**필수 env 목록** (Vercel Settings > Environment Variables):

| Key | Env | 출처 |
|---|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Prod/Preview/Dev | `.env.local` 그대로 |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Prod/Preview/Dev | `.env.local` 그대로 (로테이트 후 교체) |
| `SUPABASE_SERVICE_ROLE_KEY` | Prod/Preview | `.env.local` 그대로 (로테이트 후 교체) |
| `NEXT_PUBLIC_APP_URL` | Prod | `https://<your-domain>` |
| `SESSION_HASH_SECRET` | Prod/Preview | `openssl rand -base64 48` 로 신규 생성 |
| `CRON_SECRET` | Prod | `openssl rand -base64 32` 로 신규 생성 |
| `ADMIN_TOKEN` | Prod | `openssl rand -base64 32` 로 신규 생성 |
| `DEV_COUNTRY_OVERRIDE` | (비워둠) | 프로덕션에서는 미설정 |
| `NEXT_PUBLIC_PLAUSIBLE_DOMAIN` | Prod (선택) | `onequestionaday.com` 같은 도메인 |
| `SENTRY_DSN`, `NEXT_PUBLIC_SENTRY_DSN` | Prod (선택) | Sentry 프로젝트 생성 후 |
